...
class Controller_Admin extends Controller {
	...
	private function frontends()
	{
		...
		$query2 = DB::query(Database::UPDATE,
			"UPDATE frontends " . "SET    use_global = 1 " . "      ,show_inst = 1 " . "WHERE  last_active < DATE_SUB(NOW(),INTERVAL 5 MINUTE)"
			)->execute(__db);
		// Frontendit, jotka eivät ole ilmoittaneet itsestään yli viikkoon, poistetaan automaattisesti
		$query3 = DB::query(Database::DELETE,
			"DELETE FROM frontends " . "WHERE  last_active < DATE_SUB(NOW(),INTERVAL 1 WEEK)"
			)->execute(__db);
		// ------------------------------------------------------------------------------------------------------------------------------------------
		$this->view->content->text->tablebody = "";
		if ($result) {
	...
	private function logi()
	{
		...
		$rows = DB::query(Database::SELECT,
			'SELECT   id ' . '        ,tag ' . '        ,comment ' . '        ,adder ' . '        ,stamp ' . '        ,ack ' . '        ,ack_stamp ' . 'FROM    ' . __tableprefix . 'logi ' . 'WHERE    hidden = 0 ' . 'ORDER BY stamp DESC '
		...
		if ($tc->loaded())
			...
			foreach($odata as $row) {
				if ($row->loaded()) {
					...
					if ($row->hidden == 1)
						$checked = "checked=\"checked\"";
					$ohjelmaedit .= "<div style=\"height: 80px; width:190px; padding:5px;float:left;\"><div class=\"ui-widget-content ui-corner-all " . $row->kategoria . "\" oid=\"" . $row->id . "\" style=\"width:180px;height: 70px; padding:5px;position:absolute;\" title=\"" . htmlspecialchars($row->otsikko) . "\nPitäjä: " . $row->pitaja . "\nKategoria: " . $row->kategoria . "\nKesto: " . $row->kesto . "min\nKuvaus: " . $row->kuvaus . " \">" . $row->otsikko . "<input name=\"piilotettu\" class=\"hid toggle\" type=\"checkbox\" id=\"hidden-" . $row->id . "\" value=\"1\" " . $checked . " /><label class=\"hid\" for=\"hidden-" . $row->id . "\">Piilotettu?</label><span class=\"ui-icon ui-icon-circle-close\" style=\"float:right; position:absolute; top:3px; right:3px;\" onclick=\"del(".$row->id.")\"></span></div></div>";
				}
			}
